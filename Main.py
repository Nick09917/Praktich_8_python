        ...
        if validation_of_data(self.tableWidget):
            ...
            while i < self.tableWidget.rowCount():
                ...
                try:
                    ...
                    sum_of_k_minus_1 += int(item_minus_1)
                    sin_of_k_and_k_minus_1 = math.sin(
                        int(item) + int(item_minus_1))

                    # высчитываем ответ
                    answer = format(
                        (math.sqrt( (multiplication_of_k_i - sum_of_k_minus_1) / math.factorial(i) + math.sin(int(item)) ) / (sin_of_k_and_k_minus_1 ** 2)), ".5f")
